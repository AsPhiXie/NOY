...
void ExceptionHandler(ExceptionType exceptiontype, int vaddr)
{
 ...
 case SC_SEM_CREATE: {
   ...
   uint32_t nomSem=g_machine->ReadIntRegister(4);
   uint32_t initialValue=g_machine->ReadIntRegister(5);
   //printf("NomSem = %d\n", nomSem);
   //printf("initialValue = %d\n", initialValue);
   if(nomSem != NULL)  {
     //printf("JE SUIS DANS LE IF\n");
     int sizep;
     sizep = GetLengthParam(nomSem);
     char name[sizep];
     GetStringParam(nomSem,name,sizep);
     //printf("name = %s\n", name);
     Semaphore* result = new Semaphore(name, (int)initialValue);
     int32_t resFinal = g_object_ids->AddObject(result);
     //printf("resFInal = %d\n", resFinal);
     g_machine->WriteIntRegister(2,resFinal);
     g_syscall_error->SetMsg((char*)"",NO_ERROR);
   }
   else {
     //printf("JE SUIS DANS LE ELSE\n");
     g_machine->WriteIntRegister(2,ERROR);
     g_syscall_error->SetMsg((char*)"",INVALID_SEMAPHORE);
   }
   break;
 }
 case SC_SEM_DESTROY: {
   DEBUG('e', (char*)"Noyau: Appel à Sem_Destroy().\n");
   uint32_t semID = g_machine->ReadIntRegister(4);
   if(semID != NULL) {
     Semaphore* sem = (Semaphore*) g_object_ids->SearchObject(semID);
     if(sem != NULL && sem->type == SEMAPHORE_TYPE) {
       delete sem;
       g_machine->WriteIntRegister(2,0);
       g_syscall_error->SetMsg((char*)"",NO_ERROR);
     }
 ...
 case SC_P:{
   DEBUG('e', (char*)"Noyau: Appel à P().\n");
   uint32_t semID = g_machine->ReadIntRegister(4);
   if(semID != NULL) {
     Semaphore *sem = (Semaphore*) g_object_ids->SearchObject(semID);
     //printf("je suis dans le 1er if, valeur sem = %x\n", tmp);
     if(sem != NULL && sem->type == SEMAPHORE_TYPE) {
       //printf("je suis dans le 2eme if\n");
       sem->P();
       //A modifier ici
 ...
 case SC_LOCK_CREATE:{
   ...
   uint32_t nomLock=g_machine->ReadIntRegister(4);
   if(nomLock != NULL)  {
     int sizep;
     sizep = GetLengthParam(nomLock);
     char name[sizep];
     GetStringParam(nomLock,name,sizep);
     Lock* result = new Lock(name);
     int32_t resFinal = g_object_ids->AddObject(result);
     g_machine->WriteIntRegister(2,resFinal);
     g_syscall_error->SetMsg((char*)"",NO_ERROR);
   }
 ...
 case SC_LOCK_DESTROY:{
   DEBUG('e', (char*)"Noyau: Appel à Lock_Destroy().\n");
   uint32_t LockID = g_machine->ReadIntRegister(4);
   if(LockID != NULL) {
 ...
 case SC_LOCK_ACQUIRE:{
   DEBUG('e', (char*)"Noyau: Appel à Acquire().\n");
   uint32_t LockID = g_machine->ReadIntRegister(4);
   if(LockID != NULL) {
     Lock* lock = (Lock*) g_object_ids->SearchObject(LockID);
     if(lock && lock->type == LOCK_TYPE) {
 ...
 case SC_LOCK_RELEASE:{
   DEBUG('e', (char*)"Noyau: Appel à Release().\n");
   uint32_t LockID = g_machine->ReadIntRegister(4);
   if(LockID != NULL) {
     Lock* lock = (Lock*) g_object_ids->SearchObject(LockID);
     if(lock && lock->type == LOCK_TYPE) {
 ...
 case SC_COND_CREATE:{
   ...
   uint32_t nomCondition = g_machine->ReadIntRegister(4);
   if(nomCondition != NULL)  {
     int sizep;
     sizep = GetLengthParam(nomCondition);
     char name[sizep];
     GetStringParam(nomCondition,name,sizep);
     Condition* result = new Condition(name);
     int32_t resFinal = g_object_ids->AddObject(result);
     g_machine->WriteIntRegister(2,resFinal);
     g_syscall_error->SetMsg((char*)"",NO_ERROR);
   }
 ...
 case SC_COND_DESTROY:{
   DEBUG('e', (char*)"Noyau: Appel à ~Condition().\n");
   uint32_t ConditionID = g_machine->ReadIntRegister(4);
   if(ConditionID != NULL) {
     Condition* cond = (Condition*) g_object_ids->SearchObject(ConditionID);
 ...
 case SC_COND_WAIT:{
   DEBUG('e', (char*)"Noyau: Appel à Wait().\n");
   uint32_t ConditionID = g_machine->ReadIntRegister(4);
   if(ConditionID != NULL) {
     Condition* cond = (Condition*) g_object_ids->SearchObject(ConditionID);
     if(cond && cond->type == CONDITION_TYPE) {
 ...
 case SC_COND_SIGNAL:{
   DEBUG('e', (char*)"Noyau: Appel à Signal().\n");
   uint32_t ConditionID = g_machine->ReadIntRegister(4);
   if(ConditionID != NULL) {
     Condition* cond = (Condition*) g_object_ids->SearchObject(ConditionID);
     if(cond && cond->type == CONDITION_TYPE) {
 ...
 case SC_COND_BROADCAST:{
   DEBUG('e', (char*)"Noyau: Appel à Broadcast().\n");
   uint32_t ConditionID = g_machine->ReadIntRegister(4);
   if(ConditionID != NULL) {
     Condition* cond = (Condition*) g_object_ids->SearchObject(ConditionID);
     if(cond && cond->type == CONDITION_TYPE) {

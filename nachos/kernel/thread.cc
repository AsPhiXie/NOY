...
int Thread::Start(Process *owner, int32_t func, int arg) {
	...
	/*ASSERT(process == NULL);
	this->process = owner;
	int stackAddr = owner->addrspace->StackAllocate();
	int8_t* stackAddresse = AllocBoundedArray(SIMULATORSTACKSIZE);
	if(stackAddresse != NULL) {
		int32_t pileUser = (int32_t) stackAddr;
		InitSimulatorContext(stackAddresse, g_cfg->UserStackSize);
		InitThreadContext(func, pileUser, (int32_t) arg);
		owner->numThreads += 1;
		g_scheduler->ReadyToRun(this);
		g_alive->Append(this);
	}
	else {
		DEBUG('t', (char *)"Problème AllocBounded\n");
	}
	return NO_ERROR;*/
}
...
Thread::InitThreadContext(int32_t initialPCREG,int32_t initialSP, int32_t arg) {
    ...
    for (i = 0; i < NUM_INT_REGS; i++) {
		this->thread_context.int_registers[i] = 0;
	}

    // Initial program counter -- must be location of "Start"
    this->thread_context.int_registers[PC_REG] = initialPCREG;


    // Need to also tell MIPS where next instruction is, because
    // of branch delay possibility
    this->thread_context.int_registers[NEXTPC_REG] = initialPCREG+4;

    // Arguments
    this->thread_context.int_registers[4] = arg;

    // Set the stack register
    this->thread_context.int_registers[STACK_REG] = initialSP;
}
...
Thread::SaveProcessorState() {
	int i = 0;
	while(i<NUM_INT_REGS){
		this->thread_context.int_registers[i] = g_machine->ReadIntRegister(i);
		i++;
	}
	int j=0;
	while(j<NUM_FP_REGS){
		this->thread_context.float_registers[j] = g_machine->ReadFPRegister(j);
		j++;
	}
	this->thread_context.cc = g_machine->ReadCC();
	this->GetProcessOwner()->addrspace->translationTable = g_machine->mmu->translationTable;
}
...
Thread::RestoreProcessorState() {
	...
	int j=0;
	while(j<NUM_FP_REGS){
		g_machine->WriteFPRegister(j, this->thread_context.float_registers[j]);
		j++;
	}
